...
/** My Comment: 
 * 
    Part 1. insert() operation need lock, because insert() will write to the same bucket in the table.
    Part 3. retrieve() do not need lock, because 
         (a) read to global variable will not cause racing condition 
         (b) every thread maintains its own function stack , so changes to local variable will not cause racing condition 
         (c) we have join() to create a barrier before retrieve, so we make sure that retrieve() operation starts only after all insertions are done. 
    Part 4. In insert(), instead of using only one lock, we initializes a lock array where each lock for each bucket. The advantage is that it allows writing to different buckets in parallel.
**/
