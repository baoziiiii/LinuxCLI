...
/** My Comment:
 * 
    user@cs3224:~/Desktop/Shared$ ./mutex_bucketlock 10
        [main] Inserted 100000 keys in 0.052741 seconds
    user@cs3224:~/Desktop/Shared$ ./spin_bucketlock 10
        [main] Inserted 100000 keys in 0.035910 seconds

    user@cs3224:~/Desktop/Shared$ ./mutex_bucketlock 50
        [main] Inserted 100000 keys in 0.060338 seconds
    user@cs3224:~/Desktop/Shared$ ./spin_bucketlock 50
        [main] Inserted 100000 keys in 0.022783 seconds

    When mutex is locked for one thread, other threads will sleep until woken up when lock is released. But the sleep and wakeup is very expensive call.
    When spinlock is locked for one thread, other threads will keep looping and retrying to get the lock.
    So if one thread only keeps the lock for a liitle time, it is more expensive to use mutex because there will be frequent sleep and wakeup. 
    Our result also shows spinlock is faster, that's because we only lock for four lines of codes.
...
void insert(int key, int val) {
    ...
    // only this four lines have problem of racing condition,
    // because multiple threads may write to the same bucket of table at the same time
    e->next = table[i];
    e->key = key;
    e->val = val;
    table[i] = e;
